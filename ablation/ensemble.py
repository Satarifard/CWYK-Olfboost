...
def select_features(df, mode):
    if mode == 'class1':
        cols = [c for c in df.columns if c.startswith('Mix1_Component_')]
        return df[cols]
    elif mode == 'class23':
        cols = [c for c in df.columns if c.startswith('Mix_Comp') or c.startswith('Prediction_')]
        return df[cols]
    return df
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ensemble Inference (Ablation)")
    parser.add_argument("--ablation", type=str, required=True, choices=['class1', 'class23'],
                        help="class1: binary vectors only; class23: percept + expanded features only")
    args = parser.parse_args()

    ablation_mode = args.ablation
    model_dir = os.path.join('output', f'ablation_{ablation_mode}')
    results_dir = 'final_results'
    os.makedirs(results_dir, exist_ok=True)

    print(f"=== Ensemble Ablation: {ablation_mode} ===")
    ...
    X_leaderboard = select_features(X_leaderboard, ablation_mode)
    ...
    X_test = select_features(X_test, ablation_mode)
    X_test = applying_PolynomialFeatures(X_test)
    print("Done")

    print("Load and Ensemble Models ... ... ", end='')
    model_prefix = 'mixture_model_'
    models_and_weights = load_models_and_weights(model_dir, model_prefix)
    ...
    submission_output_path = os.path.join(results_dir, f'Leaderboard_set_Submission_form_ensemble_{ablation_mode}.csv')
    ...
    print(f'Leaderboard RMSE (ensemble {ablation_mode}): {rmse_value}')
    print(f'Leaderboard Pearson Correlation (ensemble {ablation_mode}): {pearson_corr_value}')
    print("Done")
    ...
    test_set_submission_output_path = os.path.join(results_dir, f'Test_set_Submission_form_ensemble_{ablation_mode}.csv')
