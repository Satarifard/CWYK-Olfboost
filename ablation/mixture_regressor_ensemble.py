...
def train_model(training_data_df, df_percept, mixture_definitions_df, all_cids, X_t, y_t, data_percept, percept_ids):
    ...
    X = select_features(X, ablation_mode)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Mixture Regressor (Ablation)")
    ...
    output_dir = os.path.join('output', f'ablation_{ablation_mode}')
    os.makedirs(output_dir, exist_ok=True)

    print(f"=== Ablation: {ablation_mode} | Seed: {seed} ===")
    ...
    jl.dump({'model': model, 'weight': weight}, os.path.join(output_dir, f'mixture_model_{seed}.pkl'))
    print("Done")
    ...
    submission_output_path = os.path.join(output_dir, f'Leaderboard_set_Submission_form_{seed}.csv')
    ...
    test_set_submission_output_path = os.path.join(output_dir, f'Test_set_Submission_form_{seed}.csv')
